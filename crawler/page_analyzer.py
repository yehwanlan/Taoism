...
class PageAnalyzer(BaseCrawler):
    ...
    def deep_analyze(self, url):
        ...
        for tag, desc in containers:
            ...
            if elements:
                print(f"{desc}: {len(elements)}個")
                # 找出最長的內容
                longest = max(elements, key=lambda x: len(x.get_text()))
                text = longest.get_text().strip()
                if len(text) > 50:
                    print(f"  最長內容: {len(text)} 字符")
                    print(f"  預覽: {text[:100]}...")
                    # 顯示這個元素的class和id
                    if longest.get('class'):
                        print(f"  Class: {' '.join(longest.get('class'))}")
                    if longest.get('id'):
                        print(f"  ID: {longest.get('id')}")
        print()
        
        # 4. JavaScript分析
        print("🔧 JavaScript分析:")
        scripts = soup.find_all('script')
        print(f"腳本數量: {len(scripts)}")
        ...
        for script in scripts:
            if script.string:
                ...
                if found_keywords:
                    print(f"  發現關鍵字: {', '.join(found_keywords)}")
                    if 'src' in script.attrs:
                        print(f"  外部腳本: {script['src']}")
        print()
        ...
        if content_related:
            ...
            for cls in sorted(content_related)[:10]:  # 只顯示前10個
                ...
                if elements:
                    ...
                    print(f"  .{cls}: {len(elements)}個元素, 最大內容長度: {max_text_len}")
        print()
...
def analyze_shidian_page():
    ...
    print("🎯 分析建議:")
    print("1. 檢查是否有動態載入的內容")
    print("2. 嘗試不同的內容選擇器")
    print("3. 可能需要模擬瀏覽器行為")
    print("4. 檢查是否需要特殊的請求標頭")
